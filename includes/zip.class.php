...
class ZipFile {
	...
	*/
	...
	private $files = array(); // array of uncompressed files
	private $dirs = array(); // array of directories that have been created already
	...
	/**
	* Adds a directory to the zip with the name $name
	*/
	...
	/**
	* Adds a file to the path specified by $name with the contents $data
	*/
	public static function createFile($data, $name){
		$name = str_replace("\\", "/", $name);
	...
	/**
	* Read a zip file with a $name
	*/
	public static function readZip($name){
		// Clear current file
		$this->datasec = array();
